...
- **Etapa 2A terminada** (hábitos con frecuencia propia, marcas y racha estricta). Diseño: `docs/superpowers/specs/2026-09-21-habitos-con-racha-design.md`; plan: `docs/superpowers/plans/2026-09-21-habitos-con-racha.md`. Entrega 1 (hábitos y marcas, `disciplina-v8`) confirmada por Simon en su celu; Entrega 2 (racha, `disciplina-v9`) publicada y **pendiente de prueba en el celu**. Base IndexedDB en versión 2 (store `habits`); export en `schemaVersion` 2 (los `.json` de la versión 1 se siguen pudiendo importar). Los hábitos se muestran ordenados por fecha de creación y luego por nombre (`sortHabits`). Racha: `currentStreak` en `js/logic.js`; un día pasado que tocaba y quedó sin marcar corta, hoy sin marcar es neutro.
...
- Al publicar cualquier cambio: correr `node --test`, subir `CACHE_VERSION` en `sw.js` (hoy `disciplina-v9`), commit y `git push`. Si se agrega un archivo a `js/`, `fonts/` o `icons/`, agregarlo a `ASSETS` en `sw.js` (un test lo verifica).
